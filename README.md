# Raspi-Sump for Home Assistant

Just a simple Home Assistant integration for [Raspi-Sump](https://github.com/alaudet/raspi-sump).

It appears under **Settings → Devices & Services**, creates sensor
entities, and includes a Lovelace card that reproduces the Raspi-Sump chart
inside your dashboard. I did this to replace `rest:` sensor that people have historically used
to pull readings into Home Assistant.

## Requirements

- Home Assistant **2025.2.0** or later
- A Raspi-Sump install, version **2.0.5 or later**, whose web interface is
  reachable from Home Assistant. Earlier versions do not have the
  `/api/status` endpoint this integration polls - see
  [Web Interface: JSON API](https://raspisumpdocs.linuxnorth.org/web-interface/#json-api).
...
[HACS](https://www.hacs.xyz/) is a separate community project, not part of
Home Assistant. If you do not already have a **HACS** entry in your sidebar,
you would need to [install it first](https://www.hacs.xyz/docs/use/download/download/)
- use the manual method below instead unless you want it for other reasons.

With HACS installed:

1. Open **HACS** from the sidebar.
2. Click the **⋮** menu in the **top right corner** and select **Custom repositories**.
3. Add `https://github.com/graysky2/ha-raspi-sump` and set the type to **Integration**.
...
| Core (venv) | your `--config` directory | copy it directly |
...
> **Which way is up:** Raspi-Sump records the **depth of water in the pit**,
> not the distance from the sensor down to the water - the value rises as the
> water rises. The critical binary sensor follows the same rule Raspi-Sump
> uses for alerts: it turns on above `critical_water_level` when
> `alert_when = high`, and below it when `alert_when = low`.
...
Because these are ordinary sensor entities, they also show up on the device's
own page with Home Assistant's stock history graph - no Lovelace card
required:

![The Raspi-Sump device page in the Home Assistant app, showing the built-in stat tiles and history graph for the water level sensor](images/native-device-view.png)
...
![The custom Raspi-Sump Lovelace card, reproducing the uPlot chart from the Raspi-Sump web interface](images/lovelace-card.png)
...
## Relationship to Raspi-Sump

This integration is maintained separately from
[alaudet/raspi-sump](https://github.com/alaudet/raspi-sump) - it is a client
of the appliance's `/api/status` endpoint, not part of that project. Issues
with this integration or the Lovelace card belong here; issues with the
Raspi-Sump appliance itself (readings, alerts, the web interface) belong in
[its own tracker](https://github.com/alaudet/raspi-sump/issues).

## License

Apache-2.0, matching Raspi-Sump.
